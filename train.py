...
pretrained_model_path = "./-result_path/models/best.pkl"
...
datasets = ['./data/train/images', r'./data/train/labels/label.txt']
valid_datasets = ['./data/val/images', './data/val/labels/label.txt']
...
for eidx in tqdm(range(start_epoch, max_epochs)):
    print('epoch', eidx)
    n_samples = 0
    ud_epoch = time.time()
    ...
    loss_s2 /= dispFreq
    loss_kl /= dispFreq
    print('\nData', datetime.now().strftime('%Y-%m-%d %H-%M-%S'), 'Update/total: ', str(uidx)+'/'+str(len(train)), 'Cost_L2R: ', loss_s,'Cost_R2L: ', loss_s2,'KL: ', loss_kl, 'lrate ', lrate, 'eps', my_eps, 'bad_counter', bad_counter)
    loss_s2 = loss_kl = 0
    loss_s = 0.
    ...
    print('validing... \tloss', loss_valid, '\tmin_loss', np.array(history_loss).min())
    ...
    with torch.no_grad():
        ...
        for x, y in valid:
            for xx in x:
                ...
                xx_pad = torch.from_numpy(xx_pad[None, None, :, :]).cuda()
    ...
    if valid_stop == False:
        ...
        print("valid_result: ", valid_result[0])
        stuff = fpp.readlines()
        fpp.close()
        m = re.search('WER (.*)\n', stuff[0])
        valid_err = float(m.group(1))
        m = re.search('ExpRate (.*)\n', stuff[1])
        valid_sacc = float(m.group(1))
        ...
        if valid_sacc > max_ExpRate:
            ...
            if multi_gpu_flag:
                torch.save(model.module.state_dict(), save_model_path + '/best_' + str(eidx) + '.pkl')
            else:
                torch.save(model.state_dict(), save_model_path + '/best_' + str(eidx) + '.pkl')
