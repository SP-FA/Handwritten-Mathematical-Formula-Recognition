...
def my_dataIterator(feature_file, label_file, dictionary):
    """
    feature_file 为存放 图片 的文件夹路径
    label_file 为存放 标签 的文件路径
    """
    features = {}
    fileList = os.listdir(feature_file)
    for i in fileList:
        img = Image.open(feature_file + "/" + i)
        features[i] = np.array(img)
        img.close()
    # print(features)
...
def prepare_data_bidecoder(options, images_x, seqs_y):
    ...
    for idx, [s_x, s_y] in enumerate(zip(images_x, seqs_y)):
        s_x = Image.fromarray(s_x)
        s_x = s_x.convert('L')
        s_x = np.array(s_x).astype(np.float32)
        s_x = torch.from_numpy(s_x[None, :, :])
        s_x = np.transpose(s_x, [0, 1, 2])
...
def _gen_sample_bidirection(model, x, params, gpu_flag, k, maxlen, idx_decoder):
    sample = []
    sample_score = []
    live_k = 1
    dead_k = 0
...
def gen_sample_bidirection(model, x, params, gpu_flag, k=1, maxlen=30, idx_decoder=1):
    # _gen_sample_bidirection(model, x, params, gpu_flag, k, maxlen, idx_decoder)
    sample = []
    sample_score = []
    live_k = 1
    dead_k = 0
...
if __name__ == "__main__":
    ...
    dataIterator("../data/train/images", "../data/train/labels/label.txt", worddicts, 32, 1280000, 20000, 500000)
